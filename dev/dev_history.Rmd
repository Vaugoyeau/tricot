...
#' @param h Expected height (cm) of knitting
#' @param w Expected width (cm) of knitting
#' @param p Weight to make the test square of 10*10
#' @inheritParams dplyr::arrange 
#' @inheritParams magick::image_raster
#' @inheritParams magick::image_scale
#' @inheritParams ggplot2::ggplot
...
#'
#' @examples
whool_weigth_needed <- function(img_k, h, w, p) {
  
  if(sum(class(img) != "ggplot") > 0) {stop("img must be ggplot")}
  if (!is.numeric(h)) {stop("h should be numeric")}
  ...
  if (length(gs) != 1) {stop("p should have only one value")}
  
  knit_area <- h*w
  
  # Stop ici, Laëtitia a finie sa sieste !!!!
...
ss <- square_size(35, 26)

gs <- grid_size(20, 20, ss)

img_k <- knitting_image(
  img,
  gs
)
...
sum(dplyr::count(img_k$data, col)$n)
